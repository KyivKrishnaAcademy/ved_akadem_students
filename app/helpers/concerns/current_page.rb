...
module CurrentPage
  ...
  def current_path?(uri, options = {})
    ...
    url_string = URI.decode_www_form_component(url_for(uri)).force_encoding(Encoding::BINARY)

    # We ignore any extra parameters in the request_uri if the
    # submitted url doesn't have any either. This lets the function
    # work with things like ?order=asc
    request_uri = url_string.index("?") || options[:enforce_params] ? request.fullpath : request.path
    request_uri = URI.decode_www_form_component(request_uri).force_encoding(Encoding::BINARY)
