...
module VedicAcademyStudents
  class Application < Rails::Application
    ...
    config.autoload_paths += %w[forms].map { |type| Rails.root.join('app', type) }
    config.autoload_paths += %w[helpers interactions forms].map { |type| Rails.root.join('app', type, 'concerns') }
