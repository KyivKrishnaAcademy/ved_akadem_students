...
describe 'class_schedules/index' do
  ...
  Given do
    allow(view).to receive(:policy).with(class_schedule).and_return(ClassSchedulePolicy.new(user, class_schedule))
  end

  Given do
    allow(view)
      .to receive(:policy).with(class_schedule.course).and_return(CoursePolicy.new(user, class_schedule.course))
  end

  Given do
    allow(view)
      .to receive(:policy).with(teacher_profile.person).and_return(PersonPolicy.new(user, teacher_profile.person))
  end
  ...
  describe 'conditional links' do
    ...
    Given(:no_edit_link) do
      expect(table_container).not_to have_link(I18n.t('links.edit'), href: edit_class_schedule_path(class_schedule))
    end

    Given(:no_destroy_link) do
      expect(table_container).not_to have_link(I18n.t('links.delete'), href: "/class_schedules/#{class_schedule.id}")
    end
    ...
    Given(:no_person_link) do
      expect(row)
        .not_to have_link(complex_name(teacher_profile.person, true), href: person_path(teacher_profile.person))
    ...
    context 'with :edit rights' do
      ...
      Then do
        expect(table_container).to have_link(I18n.t('links.edit'), href: edit_class_schedule_path(class_schedule))
      end
    ...
    context 'with :destroy rights' do
      ...
      Then do
        expect(table_container).to have_link(I18n.t('links.delete'), href: "/class_schedules/#{class_schedule.id}")
      end
    ...
    context 'with person:show rights' do
      ...
      Then do
        expect(row)
          .to have_link(complex_name(teacher_profile.person, true), href: person_path(teacher_profile.person))
      end
