...
describe 'people/show.html.haml' do
  ...
  describe 'general' do
    ...
    And { expect(rendered).to have_text(/Telephone 1:\s*#{Regexp.escape(person.telephones.first.phone)}/) }
    And { expect(rendered).to have_text(/Email:\s*#{Regexp.escape(person.email)}/) }
    And { expect(rendered).to have_text(/Gender:\s*(Male|Female)/) }
    And { expect(rendered).to have_text(/Birthday:\s*#{Regexp.escape(person.birthday.to_s)}/) }
  end
